...
-- This class provides the following methods:
    --    Lock()
    --         Acquire the monitormutex if free, otherwise wait until the monitormutex is
    --         free and then get it.
    --    Unlock()
    --         Release the monitormutex.  If other threads are waiting, then
    --         wake up the oldest one and give it the lock.
    --    Init()
    --         Each monitormutex must be initialized.

    --    LockTo()
    --         Give the monitormutex to the thread which will be woken up by the thread running the signal function
...
------------------------ MONITORMUTEX . LOCKTO ---------------------------
      ...
      method LockTo(threadptr: ptr to Thread)
...
----------------------- MONITORMUTEX . ISHELDBYCURRENTTHREAD ----------------
...
-------------------------- HOARE CONDITION ----------------------------
    ...
    -- means that in the time between a Signal() operation and the awakening
    -- and execution of the corrsponding waiting thread, other threads should 
    -- not snuck in and acquire the lock. The thread which called the wait function 
    -- first should acquire the lock. The waiting thread should always re-check the
    -- data to ensure that the condition which was signalled is still true.
    --
    -- This class provides the following methods:
    --    Wait(monitormutex)
    --         This method assumes the monitormutex has already been locked.
    --         It unlocks it, and goes to sleep waiting for a signal on
    --         this condition.  When the signal is received, this method
    --         re-awakens, re-locks the monitormutex, and returns.
    --    Signal(monitormutex)
    --         If there are any threads waiting on this condition, this
    --         method will wake up the oldest and schedule it to run.
    --         However, since this thread holds the mutex and never unlocks
...
----------------------- HOARECONDITION . WAIT --------------------------
      ...
      method Wait (mutex_lock: ptr to MonitorMutex)
          var
            oldIntStat: int
          oldIntStat = SetInterruptsTo (DISABLED)
          if !mutex_lock.IsHeldByCurrentThread ()
            FatalError ("Attempt to wait on condition when mutex is not held")
          endIf
...
---------------------- HOARECONDITION SIGNAL ---------------------------
      ...
      method Signal (mutex_lock: ptr to MonitorMutex)
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        var 
          ...
          threadTable = new array of Thread {MAX_NUMBER_OF_PROCESSES of new Thread } -- Initializing the array which stores the threads
          freeList = new List[Thread]             -- Initializing the freeList which will store the UNUSED threads


          aThreadBecameFree  = new Condition
          aThreadBecameFree.Init()      -- Initializing the condition variable
          threadManagerLock = new Mutex           -- Initializing the mutex lock
          ...
          -- Updating the status of all the threads in threadTable array to UNUSED and storing them in the freelist 
      ...
      method GetANewThread () returns ptr to Thread
        ...
        var
          threadpointer: ptr to Thread 
        

        threadManagerLock.Lock()

        -- Check if the freelist is epmty. If it is empty then wait on the condition of a thread becoming available
        ...
        -- if the freelist is not empty then remove a thread from the freeList and change its status to JUST_CREATED
        -- This newly created thread is returned to the pointer requesting it in the ThreadManagerTests function in MAIN.C file
          
        threadpointer = freeList.Remove()
        threadpointer.status = JUST_CREATED
      ...
      method FreeThread (th: ptr to Thread)
        ...
        -- To free the thread change the status of the calling thread to UNUSED and add it to the end of freeList
        -- After adding the thread in the freelist this function will signal any thread waiting on the condition.
        
        -- It uses Mesa-style semantics in which between the time of signal call and actual awakening of the corresponding thread
        -- other threads can swoop in and try to acquire the lock

        -- To implement Hoare-style semantics, initialize the condition variables to HOARECONDITION and locking variables to MONITORMUTEX
        -- in kernel.c and kernel.h files
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        processTable = new array of ProcessControlBlock {MAX_NUMBER_OF_PROCESSES of new ProcessControlBlock} ---- Initializing the array which stores the ProcessControlBlocks of each process

        freeList = new List[ProcessControlBlock] -- initialize a new freelist to store the FREE ProcessControlBlocks
        
        processManagerLock = new Mutex
        processManagerLock.Init()

        aProcessBecameFree = new Condition        -- Condition variable to signal if a processcontrolblock/process becomes free 
        aProcessBecameFree.Init()

        aProcessDied = new Condition              -- Condition variable to signal if a ProcessControlBlock/process died
        aProcessDied.Init()
        ...
        -- initialize all the ProcessControlBlocks in the processTable array to FREE and store it in the freeList
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        var
          processpointer: ptr to ProcessControlBlock 
        ...
        -- Check if the freelist is empty. If it is empty then wait on the condition of a ProcessControlBlock becoming available
        ...
        -- if the freelist is not empty then remove a ProcessControlBlock from the freeList and change its status to ACTIVE
        -- This newly activated ProcessControlBlock is returned to the pointer requesting it in the ProcessManagerTests function in MAIN.C file

        processpointer = freeList.Remove()
        processpointer.status = ACTIVE


          if processpointer.status == ZOMBIE
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- To free the ProcessControlBlock change the status of the calling process to FREE and add it to the end of freeList
        -- After adding the ProcessControlBlock in the freelist this function will signal any thread waiting on the condition.
        -- It uses Mesa-style semantics in which between the time of signal call and actual awakening of the corresponding ProcessControlBlock
        -- other processes can swoop in and try to acquire the lock
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      -- Allocate the number of frames needed and return its physical address. If there are no free frames currently available
      ...
      -- Wait on the condition if the requirement is more than the available free frames

      while numberFreeFrames < numFramesNeeded
        newFramesAvailable.Wait(& frameManagerLock)
      endWhile

      -- determine which frames are free and store it in a variable. Use this value of the variable to calculate the physical address 
      -- of the free frame
      ...
      -- adjusting the number of free frames now available after allocating the frames needed which is called by a pointer
      -- to the processcontrolblock in Main.C file

      numberFreeFrames = numberFreeFrames - numFramesNeeded
      aPageTable.numberOfPages = numFramesNeeded -- store the number of pages allocated
      ...
      -- Returning all the frames together which were allocated in the aboe function

      var
        i, numFramesReturned, f, frameAddr: int

      frameManagerLock.Lock()

      -- store the count of the number of allocated frames so that we can free them

      numFramesReturned = aPageTable.numberOfPages

      -- Compute and return the frame number which can be calculated using the physical address of the frame that needs to freed
      -- the frames can be freed by changing the bit to 0 

      for i = 0 to numFramesReturned - 1
        frameAddr = aPageTable.ExtractFrameAddr(i)
        f = (frameAddr - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME)/PAGE_SIZE
        framesInUse.ClearBit(f)
      endFor

      -- adjusting the number of free frames now available after freeing the frames which were allocated before 

      numberFreeFrames = numberFreeFrames + numFramesReturned

      -- using broadcast to wake up all the threads because the thread which is waiting on the condition may not have 
      -- enough threads to complete execution 
